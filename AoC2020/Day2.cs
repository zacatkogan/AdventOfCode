    ...
    internal class Day2
    {
        ...
        public static void PartOne()
        {
            ...
            foreach (var input in GetInput())
            {
                ...
                var occurrence = segments[0].Split('-');
                var occurrenceLower = int.Parse(occurrence[0]);
                var occurrenceUpper = int.Parse(occurrence[1]);

                var letter = segments[1][0];

                var password = segments[2];

                var numberOfOccurrences = password.Count(x => x.Equals(letter));
                if (numberOfOccurrences >= occurrenceLower && numberOfOccurrences <= occurrenceUpper)
                {
                    ValidPasswords.Add(input);
                }
            }
        ...
        public static void PartTwo()
        {
            ...
            foreach (string input in GetInput())
            {
                ...
                var occurrence = segments[0].Split('-');
                var firstOccurrence = int.Parse(occurrence[0]);
                var secondOccurrence = int.Parse(occurrence[1]);

                var letter = segments[1][0];

                var password = segments[2];

                var firstLetter = password[firstOccurrence - 1];
                var secondLetter = password[secondOccurrence - 1];
        ...
        static string rawInput = @"16-18 h: hhhhhhhhhhhhhhhhhh
