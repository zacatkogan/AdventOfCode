...
namespace AdventOfCode.AOC2020
{
    internal class Day4
    {
        public static string[] RequiredFields = new[] { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" };

        public static void PartOne()
        {
            var validCount = 0;

            var passportRawData = SplitRawDataByPassport();//.ToList();

            foreach (var passportLine in passportRawData)
            {
                var kvp = passportLine.Split(' ');
                var passport = kvp.Select(x => x.Split(':')).ToDictionary(x => x[0], x => x[1]);

                var missingFields = RequiredFields.Except(passport.Keys);
        ...
        {
            var validCount = 0;
            foreach (var passport in GetPassports(ValidPassports))
            {
                // check if we're missing any required fields
                if (RequiredFields.Except(passport.Keys).Any())
                    continue;

                // validate byr
                if (!(int.TryParse(passport["byr"], out int birthYear) && birthYear >= 1920 && birthYear <= 2002))
                    continue;

                if (!(int.TryParse(passport["iyr"], out int issueYear) && issueYear >= 2010 && issueYear <= 2020))
                    continue;

                if (!(int.TryParse(passport["eyr"], out int expYear) && expYear >= 2020 && expYear <= 2030))
                    continue;

                var heightRaw = passport["hgt"];
                if (heightRaw.EndsWith("cm"))
                {
                    if (!(int.TryParse(heightRaw.Substring(0, 3), out int height) && height >= 150 && height <= 193))
                        continue;
                }
                else if (heightRaw.EndsWith("in"))
                {
                    if (!(int.TryParse(heightRaw.Substring(0, 2), out int height) && height >= 59 && height <= 76))
                        continue;
                }
                else
                    continue;

                if (!Regex.IsMatch(passport["hcl"], "#[a-f0-9]{6}"))
                    continue;
        ...
        public static IEnumerable<IDictionary<string, string>> GetPassports()
        {
            return SplitRawDataByPassport().Select(x => x.Split(' ').Select(x => x.Split(':')).ToDictionary(x => x[0], y => y[1]));
        }

        public static IEnumerable<IDictionary<string, string>> GetPassports(string data)
        {
            return SplitRawDataByPassport(data).Select(x => x.Split(' ').Select(x => x.Split(':')).ToDictionary(x => x[0], y => y[1]));
        }

        public static IEnumerable<string> SplitRawDataByPassport()
        {
            return SplitRawDataByPassport(rawData);
        }


        public static IEnumerable<string> SplitRawDataByPassport(string data)
        {
            var lines = data.Split("\r\n");

            List<string> passportDataRaw = new List<string>();
            foreach (var row in lines)
            {

                if (!string.IsNullOrEmpty(row))
                    passportDataRaw.Add(row);
                else
                {
                    if (!passportDataRaw.Any())
                        continue;
                    var tmp = string.Join(' ', passportDataRaw);
                    passportDataRaw.Clear();
                    yield return tmp;
                }
            }

            yield return string.Join(' ', passportDataRaw);

        }



        public static string testData = @"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in";
        ...
        public static string InvalidPassports = @"eyr:1972 cid:100
...
pid:3556412378 byr:2007";
        ...
        public static string ValidPassports = @"pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
