namespace AdventOfCode.AoC2021
{
    ...
    public class Day_13 : BaseDay
    {
        ...
        {
            var splitIndex = DataLines.Select((x, i) => (index: i, content: x)).First(x => string.IsNullOrEmpty(x.content)).index;

            var dots = DataLines.Take(splitIndex).Select(i => i.Split(",")).Select(i => (x: int.Parse(i[0]), y: int.Parse(i[1]))).ToList();

            var folds = DataLines.Skip(splitIndex + 1).ToList();
